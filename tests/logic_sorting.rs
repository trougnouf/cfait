...
fn test_sorting_priority_basic() {
    ...
    // 1 < 9
    assert_eq!(
        high.compare_with_cutoff(&low, None),
        std::cmp::Ordering::Less
    );

    // 1 < 0 (High vs None/Normal)
    assert_eq!(
        high.compare_with_cutoff(&none, None),
        std::cmp::Ordering::Less
    );
}
...
fn test_sorting_status_trumps_everything() {
    ...
    assert_eq!(
        active.compare_with_cutoff(&critical, None),
        std::cmp::Ordering::Less
    );
}
...
fn test_sorting_completed_sinks() {
    ...
    assert_eq!(
        todo.compare_with_cutoff(&done, None),
        std::cmp::Ordering::Less
    );
}
...
fn test_sorting_due_dates() {
    ...
    // Soon < Later
    assert_eq!(t1.compare_with_cutoff(&t2, None), std::cmp::Ordering::Less);

    // Date < No Date
    assert_eq!(t2.compare_with_cutoff(&t3, None), std::cmp::Ordering::Less);
}
...
fn test_hierarchy_organization() {
    ...
    let organized = Task::organize_hierarchy(tasks, None);
