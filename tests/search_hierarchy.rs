...
fn test_child_match_does_not_force_parent_if_parent_does_not_match() {
    ...
    if let Some(cfait::store::TaskListItem::Task(child_task)) = child_result {
        assert!(
            !child_task.is_search_context,
            "Child should not be search context"
        );
    }
    ...
    assert!(
        ...
        "Parent should be in results as context"
    );
    if let Some(cfait::store::TaskListItem::Task(parent_task)) = parent_result {
        assert!(
            parent_task.is_search_context,
            "Parent should be search context"
        );
    }
}
...
fn test_sibling_match_only_includes_matching_sibling() {
    ...
    if let Some(cfait::store::TaskListItem::Task(child_task)) = child1_result {
        assert!(
            !child_task.is_search_context,
            "Child1 should not be search context"
        );
    }
