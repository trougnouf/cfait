...
Cfait should compile and pull dependencies automatically on most systems as long as `cargo` (rust) is installed. A Vulkan driver is recommended for hardware acceleration. A keyring (s.a. `kwallet` or `gnome-keyring`) is needed to save the CalDAV password. `xdg-desktop-portal` (optional) is needed by the file picker (to import/export a local collection) on Linux. For a precise list of dependencies, refer to [packaging/arch/PKGBUILD](packaging/arch/PKGBUILD).
