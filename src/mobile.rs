...
impl CfaitMobile {
    ...
    pub fn write_widget_snapshot(
        ...
        let selected_cats: HashSet<String> = filter_tags.into_iter().collect();
        let selected_locs: HashSet<String> = filter_locations.into_iter().collect();
        ...
        // Build a calendar color lookup for the snapshot.
        let cal_colors: HashMap<String, Option<String>> = self
            .get_calendars()
            .into_iter()
            .map(|c| (c.href, c.color))
            .collect();
        ...
        for item in &filtered.items {
            ...
            if let crate::store::TaskListItem::Task(t) = item {
                ...
                if !is_done {
                    ready += 1;
                    if t.is_due_today || t.is_overdue {
                        overdue += 1;
                    }
                }
                let color = cal_colors.get(&t.calendar_href).cloned().flatten();
                let summary = task_to_summary(t, &store);
                tasks_json.push(serde_json::json!({
                    ...
                    "is_blocked": t.is_blocked,
                    "priority": t.priority,
                    "due_date_iso": summary.due_date_iso,
                    "is_due_today": t.is_due_today,
                    "is_allday_due": summary.is_allday_due,
        ...
        // Collect ongoing tasks (timers running).
        let mut ongoing_json = Vec::new();
        for (href, map) in &store.calendars {
            if href == crate::storage::LOCAL_TRASH_HREF || href == "local://recovery" {
                continue;
            }
            for t in map.values() {
                if t.status == crate::model::TaskStatus::InProcess {
                    ongoing_json.push(serde_json::json!({
                        "uid": t.uid,
                        "summary": t.summary,
                        "last_started_at": t.last_started_at,
                    }));
        ...
        let snapshot = serde_json::json!({
            "generated_at": now_ts,
            "tasks": tasks_json,
            "ongoing": ongoing_json,
            "counts": {
                "ready": ready,
                "overdue": overdue,
                "ongoing": ongoing_json.len() as u32,
            },
        });

        let dest = std::path::Path::new(&path);
        if let Some(parent) = dest.parent() {
            std::fs::create_dir_all(parent).map_err(|e| MobileError::from(e.to_string()))?;
        }
        ...
        .map_err(|e| MobileError::from(e.to_string()))?;
        std::fs::rename(&tmp, dest).map_err(|e| MobileError::from(e.to_string()))?;
