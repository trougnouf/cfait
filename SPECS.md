...
*   **Document Preservation & Transient Metadata:** 
    *   **VTODO (Tasks & Structural Notes):** For standard tasks (`VTODO`), Markdown Headers (`# Header`) and plain bullets (`- plain text`) *are* extracted into distinct structural `VTODO` components (flagged with `is:note`). This allows rapidly building hierarchical task trees without managing indentation spaces. Sub-items of a `VTODO` note are always parsed as independent components.
    *   **VJOURNAL (Wiki Pages & Journals):** For wiki pages (`is:page` / `is:journal`), long-form descriptions (Markdown headers, plain text, and plain lists) are **never** extracted into distinct components. They remain safely intact inside the `DESCRIPTION` property to prevent shredding paragraphs. Sub-items of a `VJOURNAL` are treated as part of the document body unless they contain actionable checkboxes (`- [ ]`), which are always extracted as `VTODO` sub-tasks.
    *   **Tree Serialization:** When editing an entire tree (`Ctrl+E`), `VJOURNAL` components are serialized explicitly as `- Page Title is:page` (or `is:journal`). This unambiguous token guarantees that re-parsing the tree preserves the component type and correctly scopes the "journal context" for all nested children. `VTODO` structural notes are simply serialized as `- Note Title`.
    *   **Context-Aware Parser:** The extraction parser tracks the "journal" context dynamically based on indentation. Any sub-tree indented under a `VJOURNAL` node is parsed using `VJOURNAL` rules (no shredding of plain text).
    *   **Transient Metadata:** Any smart syntax tags (`#tag`) or locations (`@@loc`) written inside a journal's description body *are* dynamically extracted into transient memory. This ensures the journal page appears under those categories in the Sidebar and Search results, while preventing ghost tags from polluting the permanent CalDAV `CATEGORIES` property.
...
*   **Smart Commands:** If the Add Task input starts with `:` and contains no spaces (e.g., `:undo`, `:redo`, `:empty-trash`, `:login`), it is intercepted and executed as a session command rather than creating a task. 
