...
*   **Tag Matching:** Goals match a task's explicit tags/locations only (no parent-chain walk). Tags are copied onto subtasks at creation time; removing a tag from a subtask is a deliberate opt-out from the parent's goal.
*   **Cascade Dedup:** Starting a subtask auto-starts all ancestors, producing overlapping `WorkSession`s on both the subtask and its parents. To avoid double-counting, descendant session intervals are subtracted from a parent's session — the overlapping portion is counted once (by the descendant), and the parent keeps its independent remainder. A parent that continues running after a subtask is paused still gets credit for the gap. Leaf tasks short-circuit (no descendants).
